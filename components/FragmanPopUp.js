...
const FragmanPopUp = ({ videoLink, setPopUpIsActive }) => {
	...
	return (
		<div
			className="fixed top-0 left-0 h-screen w-screen bg-black/90 z-40 flex items-center justify-center animate-fate-in-speed"
			onClick={() => setPopUpIsActive(false)}>
			<div className={isVideoLoading ? "block" : "hidden"}>
				<h3 className="text-white text-3xl">Loading...</h3>
			</div>
			<div
				className={isVideoLoading ? "hidden" : "lg:w-1/2 md:w-2/3 w-full px-3"}>
				<VimeoPlayer
					videoLink={videoLink}
